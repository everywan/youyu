...
3. 点击汇总行展开列表明细。
4. 明细默认包含“汇总项”和餐饮、房租/房贷、通勤、水电燃气、通讯网络、医疗健康、保险、家庭/赡养、娱乐购物、零花钱。
...
预算默认使用子项录入。页面默认只展示三档汇总行，基础 / 舒适 / 理想默认月总支出为 `4000 / 8000 / 10000`。点击汇总行后展开明细，明细默认包含“汇总项”和餐饮、房租/房贷、通勤、水电燃气、通讯网络、医疗健康、保险、家庭/赡养、娱乐购物、零花钱；所有明细项都可以删除，用户可以追加任意自定义 `key + value` 子项。`monthlyFixed` 由所有预算子项自动汇总为月总支出，不再单独录入总固定支出、日常、家庭、耐用预算字段。
...
首页保留最近一个月现金流的月结余作为现实现金流状态。预计自由时间使用三档预算门槛、负债月供、月现金收入和所有收益型资产整体年化推演；没有现金流记录时，预计自由时间显示为缺少数据。
...
下月收益型资产总额 = 当前收益型资产总额 + 月现金收入 + 本月资产收入 - 对应档位月预算 - 负债月供
```
...
2. 负债不从收益型资产总额里再次扣减收益基数；负债影响通过负债月供进入每月现金流。
3. 预留资产和不可动用资产保留为可支配口径展示和风险提示，不阻止其按当前整体年化参与资产收益计算。

边界：

1. 当前已经达标，状态为 `achieved`。
2. 扣除对应档位月预算和负债月供后现金流无法自然达成，状态为 `not_reachable`。
...
3. 高级自由时间。
4. 与当前状态相比提前或推迟多久。
5. 推演过程：收益型资产总额、整体实际年化、月现金收入、本月资产收入、该档月预算、负债月供、预算后净流入。
6. 当前主要瓶颈。
...
8. 删除资产、负债、现金流前需要确认；预算明细项可以直接删除。
9. 清空本地数据需要二次确认。
